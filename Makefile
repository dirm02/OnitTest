...
quickstart: install docker-up
	...
	cd backend && uv run onit_test db migrate -m "initial" || true
	@echo "Applying migrations..."
	cd backend && uv run onit_test db upgrade
	@echo "Creating admin user (admin@example.com / admin123)..."
	cd backend && uv run onit_test user create --email admin@example.com --password admin123 --superuser 2>/dev/null || true
	@echo ""
...
db-migrate:
	...
	uv run --directory backend onit_test db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend onit_test db upgrade

db-downgrade:
	uv run --directory backend onit_test db downgrade

db-current:
	uv run --directory backend onit_test db current

db-history:
	uv run --directory backend onit_test db history

# === Server ===
run:
	uv run --directory backend onit_test server run --reload

run-prod:
	uv run --directory backend onit_test server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend onit_test server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend onit_test user create-admin

user-create:
	uv run --directory backend onit_test user create

user-list:
	uv run --directory backend onit_test user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/onit_test"
...
help:
	@echo ""
	@echo "onit_test - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Quick Start:"
