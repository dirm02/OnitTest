...
```bash
cd backend
uv run onit_test <group> <command> [options]
```

### Server Commands

```bash
uv run onit_test server run              # Start dev server
uv run onit_test server run --reload     # With hot reload
uv run onit_test server run --port 9000  # Custom port
uv run onit_test server routes           # Show all registered routes
```

### Database Commands

```bash
uv run onit_test db init                  # Run all migrations
uv run onit_test db migrate -m "message"  # Create new migration
uv run onit_test db upgrade               # Apply pending migrations
uv run onit_test db upgrade --revision e3f  # Upgrade to specific revision
uv run onit_test db downgrade             # Rollback last migration
uv run onit_test db downgrade --revision base  # Rollback to start
uv run onit_test db current               # Show current revision
uv run onit_test db history               # Show migration history
```
...
uv run onit_test user create

# Create user non-interactively
uv run onit_test user create --email user@example.com --password secret

# Create user with specific role
uv run onit_test user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run onit_test user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run onit_test user create-admin --email admin@example.com --password secret

# Change user role
uv run onit_test user set-role user@example.com --role admin

# List all users
uv run onit_test user list
```
...
```bash
uv run onit_test cmd <command-name> [options]
```
...
```bash
uv run onit_test cmd my-command --name test
```
